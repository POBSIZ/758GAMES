...
export default class Tobacco extends Phaser.Physics.Arcade.Sprite {
	// 	초기화
	constructor(scene, x, y, player) {
		...
		// 	캐릭터 설정
		this.setTexture('tobacco_right_run');
	...
	handleMovement() {
		...
		if (this.player.x < this.x && this.active) {
			this.play('tobacco_left_run', true);
		} else if (this.player.x > this.x && this.active) {
			this.play('tobacco_right_run', true);
		}
	}
}